...
func (s *testParserSuite) TestDMLStmt(c *C) {
	table := []testCase{
		...
		{"ROLLBACK TO abc", true, "ROLLBACK TO `abc`"},
		{"SAVEPOINT abc", true, "SAVEPOINT `abc`"},
		{`BEGIN;
...
func (s *testParserSuite) TestWindowFunctions(c *C) {
	table := []testCase{
		...
		{`SELECT COUNT(*) OVER() AS country_profit FROM sales;`, true, "SELECT COUNT(*) OVER () AS `country_profit` FROM `sales`"},
