...
// Restore implements Node interface.
func (n *RollbackToStmt) Restore(ctx *format.RestoreCtx) error {
	ctx.WriteKeyWord("ROLLBACK TO ")
	ctx.WriteName(n.Savepoint)
	return nil
}

// Accept implements Node Accept interface.
func (n *RollbackToStmt) Accept(v Visitor) (Node, bool) {
	newNode, skipChildren := v.Enter(n)
	if skipChildren {
		return v.Leave(newNode)
	}
	n = newNode.(*RollbackToStmt)
	return v.Leave(n)
}
