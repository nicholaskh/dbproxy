...
func (n *ColumnName) RestoreAsPlain(ctx *format.RestoreCtx) error {
	if n.Schema.O != "" {
		ctx.WritePlain(n.Schema.O)
		ctx.WritePlain(".")
	}
	if n.Table.O != "" {
		ctx.WritePlain(n.Table.O)
		ctx.WritePlain(".")
	}
...
func (n *ColumnNameExpr) RestoreAsPlain(ctx *format.RestoreCtx) error {
	if err := n.Name.RestoreAsPlain(ctx); err != nil {
		return errors.Trace(err)
	}
	return nil
}
...
// WildCardExpr is the expression for "*" used in COUNT(*).
type WildCardExpr struct {
	exprNode
}

// Restore implements Node interface.
func (n *WildCardExpr) Restore(ctx *format.RestoreCtx) error {
	ctx.WritePlain("*")
	return nil
}

// Format the ExprNode into a Writer.
func (n *WildCardExpr) Format(w io.Writer) {
	fmt.Fprint(w, "*")
}

// Accept implements Node Accept interface.
func (n *WildCardExpr) Accept(v Visitor) (Node, bool) {
	newNode, skipChildren := v.Enter(n)
	if skipChildren {
		return v.Leave(newNode)
	}
