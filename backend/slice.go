...
type Slice struct {
	...
	MasterDCSlave          []*ConnectionPool
	MasterDCSlaveWeights   []int
	MasterDCLastSlaveIndex int
	MasterDCRoundRobinQ    []int
...
func (s *Slice) GetConn(executeFlag int, userType int) (pc *PooledConnection, err error) {
	if SlaveFlag&executeFlag == SlaveFlag {
	...
	} else if MasterDCFlag&executeFlag == MasterDCFlag {
		pc, err = s.GetMasterDCSlaveConn()
	} else {
		pc, err = s.GetMasterConn()
	}
...
func (s *Slice) GetMasterDCSlaveConn() (*PooledConnection, error) {
	s.Lock()
	cp, err := s.getNextMasterDCSlave()
	s.Unlock()
	if err != nil {
		return nil, err
	}
...
// ParseMasterDCSlave create connection pool of slaves
// (127.0.0.1:3306@2,192.168.0.12:3306@3)
func (s *Slice) ParseMasterDCSlave(slaves []string) error {
	if len(slaves) == 0 {
		return nil
	}
	...
	count := len(slaves)
	s.MasterDCSlave = make([]*ConnectionPool, 0, count)
	s.MasterDCSlaveWeights = make([]int, 0, count)
	...
	for i := 0; i < count; i++ {
		...
		s.MasterDCSlaveWeights = append(s.SlaveWeights, weight)
		idleTimeout, err := util.Int2TimeDuration(s.Cfg.IdleTimeout)
		if err != nil {
			return err
		}
		...
		cp.Open()
		s.MasterDCSlave = append(s.MasterDCSlave, cp)
	}
	s.initMasterDCBalancer()
	return nil
}
