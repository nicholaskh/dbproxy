...
// RollbackTo send ComQuery with 'rollback to #savepoint' to backend mysql to rollback transaction
func (dc *DirectConnection) RollbackTo(savepoint string) error {
	_, err := dc.exec(fmt.Sprintf("rollback to %s", savepoint))
	return err
}

// Savepoint send ComQuery with 'savepoint #savepoint' to backend mysql to rollback transaction
func (dc *DirectConnection) Savepoint(savepoint string) error {
	_, err := dc.exec(fmt.Sprintf("savepoint %s", savepoint))
	return err
}
