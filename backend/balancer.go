...
func (s *Slice) initMasterDCBalancer() {
	var sum int
	s.MasterDCLastSlaveIndex = 0
	gcd := gcd(s.MasterDCSlaveWeights)

	for _, weight := range s.MasterDCSlaveWeights {
		sum += weight / gcd
	}

	s.MasterDCRoundRobinQ = make([]int, 0, sum)
	for index, weight := range s.MasterDCSlaveWeights {
		for j := 0; j < weight/gcd; j++ {
			s.MasterDCRoundRobinQ = append(s.MasterDCRoundRobinQ, index)
		}
	}

	//random order
	if 1 < len(s.MasterDCSlaveWeights) {
		r := rand.New(rand.NewSource(time.Now().UnixNano()))
		for i := 0; i < sum; i++ {
			x := r.Intn(sum)
			temp := s.MasterDCRoundRobinQ[x]
			other := sum % (x + 1)
			s.MasterDCRoundRobinQ[x] = s.MasterDCRoundRobinQ[other]
			s.MasterDCRoundRobinQ[other] = temp
		}
	}
}
...
// getNextSlave return connection pool of calculated ip
func (s *Slice) getNextMasterDCSlave() (*ConnectionPool, error) {
	var index int
	queueLen := len(s.MasterDCRoundRobinQ)
	if queueLen == 0 {
		return nil, errors.ErrNoDatabase
	}
	if queueLen == 1 {
		index = s.MasterDCRoundRobinQ[0]
		return s.MasterDCSlave[index], nil
	}

	s.MasterDCLastSlaveIndex = s.MasterDCLastSlaveIndex % queueLen
	index = s.MasterDCRoundRobinQ[s.MasterDCLastSlaveIndex]
	if len(s.MasterDCSlave) <= index {
		return nil, errors.ErrNoDatabase
	}
	cp := s.MasterDCSlave[index]
	s.MasterDCLastSlaveIndex++
	s.MasterDCLastSlaveIndex = s.MasterDCLastSlaveIndex % queueLen
	return cp, nil
}
