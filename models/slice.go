...
type Slice struct {
	...
	SlaveDCMaster string   `json:"slave_dc_master"`
	SlaveDCSlaves []string `json:"slave_dc_slaves"`
...
func (s *Slice) verify() error {
	...
	for _, slaveDCSlave := range s.SlaveDCSlaves {
		if slaveDCSlave == "" {
			return errors.New("illegal slave dc slave addr")
		}
	}
