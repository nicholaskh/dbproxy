...
type Proxy struct {
	...
	WriteElapsedInterval int `ini:"write_elapsed_interval"` //写入延迟时间间隔

	//数据同步指定的延迟
	DcDelay int `ini:"dc_delay"` //写入延迟时间间隔
}
...
func ParseProxyConfigFromFile(cfgFile string) (*Proxy, error) {
	...
	if proxyConfig.Cluster == "" && proxyConfig.CoordinatorRoot == "" {
		...
		clusterSlice := strings.Split(proxyConfig.CoordinatorRoot, "/")
		proxyConfig.Cluster = clusterSlice[len(clusterSlice)-1]
	} else if proxyConfig.Cluster != "" {
		proxyConfig.CoordinatorRoot = "/" + core.EtcdRootPrefix + "/" + proxyConfig.Cluster
	}

	if proxyConfig.LogFormat != LogFormatJson && proxyConfig.LogFormat != LogFormatPlain {
		return nil, errors.New("The log format is incorrect! Support json and plain.")
	}
