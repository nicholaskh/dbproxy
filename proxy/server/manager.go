...
func CreateManager(cfg *models.Proxy, namespaceConfigs map[string]*models.Namespace) (*Manager, error) {
	...
	m.writeElapsedInterval = cfg.WriteElapsedInterval
	if m.writeElapsedInterval < 0 {
		err := errors.ErrWriteElapsedInterval
		log.Warn("writeElapsedInterval: %d, err: %v", m.writeElapsedInterval, err)
		return nil, err
	}
	// init statistics
...
func (m *Manager) RecordSessionSQLMetrics(reqCtx *util.RequestContext, se *SessionExecutor, sql string, startTime time.Time, err error) {
	...
	var ok bool
	if fingerprint, ok = reqCtx.Get(util.Fingerprint).(string); !ok {
	...
	// record sql timing
	m.statistics.recordSessionSQLTiming(namespace, fingerprint, startTime)
	...
	if duration > ns.getSessionSlowSQLTime() || ns.getSessionSlowSQLTime() == 0 {
		...
		m.statistics.recordSessionSlowSQLFingerprint(namespace, fingerprint)
	}
	...
	if err != nil {
		...
		m.statistics.recordSessionErrorSQLFingerprint(namespace, operation, fingerprint)
	}
...
func (m *Manager) RecordBackendSQLMetrics(reqCtx *util.RequestContext, namespace string, sql, backendAddr string, startTime time.Time, err error) {
	...
	// record sql timing
	m.statistics.recordBackendSQLTiming(namespace, fingerprint, startTime)

	// record slow sql
	duration := time.Since(startTime).Nanoseconds() / int64(time.Millisecond)
	if m.statistics.isBackendSlowSQL(startTime) {
		// FIXME Warn
		log.Notice("backend slow SQL, namespace: %s, addr: %s, sql: %s, cost: %d ms", namespace, backendAddr, trimmedSql, duration)
		md5 := mysql.GetMd5(fingerprint)
		ns.SetBackendSlowSQLFingerprint(md5, fingerprint)
		m.statistics.recordBackendSlowSQLFingerprint(namespace, fingerprint)
	}

	// record error sql
	if err != nil {
		log.Error("backend error SQL, namespace: %s, addr: %s, sql: %s, cost %d ms, err: %v", namespace, backendAddr, trimmedSql, duration, err)
		md5 := mysql.GetMd5(fingerprint)
		ns.SetBackendErrorSQLFingerprint(md5, fingerprint)
		m.statistics.recordBackendErrorSQLFingerprint(namespace, operation, fingerprint)
	}
}
...
type StatisticManager struct {
	...
	dcDelay                          *stats.MultiTimings            //延迟统计
...
func (s *StatisticManager) Init(cfg *models.Proxy) error {
	...
	s.sqlTimings = stats.NewMultiTimings("SqlTimings",
		"gaea proxy sql sqlTimings", []string{statsLabelCluster, statsLabelNamespace, statsLabelFingerprint})
	...
	s.backendSQLTimings = stats.NewMultiTimings("BackendSqlTimings",
		"gaea proxy backend sql sqlTimings", []string{statsLabelCluster, statsLabelNamespace, statsLabelFingerprint})
	...
	s.dcDelay = stats.NewMultiTimings("DcDelay",
		"dc delay", []string{statsLabelCluster, statsLabelNamespace, statsLabelSlice, statsLabelDcDelay})
...
func (s *StatisticManager) recordSessionSlowSQLFingerprint(namespace string, fingerprint string) {
	fingerprintStatsKey := []string{s.clusterName, namespace, fingerprint}
...
func (s *StatisticManager) recordSessionSQLTiming(namespace string, fingerprint string, startTime time.Time) {
	fingerprintStatsKey := []string{s.clusterName, namespace, fingerprint}
	s.sqlTimings.Record(fingerprintStatsKey, startTime)
}
...
func (s *StatisticManager) recordBackendSlowSQLFingerprint(namespace string, fingerprint string) {
	fingerprintStatsKey := []string{s.clusterName, namespace, fingerprint}
	s.backendSQLFingerprintSlowCounts.Add(fingerprintStatsKey, 1)
}

func (s *StatisticManager) recordBackendErrorSQLFingerprint(namespace string, operation string, fingerprint string) {
	fingerprintStatsKey := []string{s.clusterName, namespace, fingerprint}
	operationStatsKey := []string{s.clusterName, namespace, operation}
...
func (s *StatisticManager) recordBackendSQLTiming(namespace string, fingerprint string, startTime time.Time) {
	fingerprintStatsKey := []string{s.clusterName, namespace, fingerprint}
	s.backendSQLTimings.Record(fingerprintStatsKey, startTime)
}
