...
func NewNamespace(namespaceConfig *models.Namespace) (*Namespace, error) {
	...
	namespace := &Namespace{
		...
		masterDC:             namespaceConfig.MasterDC,
	}
	...
	// init backend slices
	namespace.slices, err = parseSlices(namespaceConfig.Slices, namespace.defaultCharset, namespace.defaultCollationID, namespaceConfig.MasterDC, namespaceConfig.ForceMaster)
	if err != nil {
	...
	namespace.masterDC = namespaceConfig.MasterDC

	return namespace, nil
}

// GetName return masterDC of namespace
func (n *Namespace) GetMasterDC() bool {
	return n.masterDC
}
...
func (n *Namespace) GetRWSplit(user string) int {
	if n.userProperties[user].RWSplit == models.ReadWriteSplit {
		return backend.SlaveFlag
	} else {
...
func parseSlice(cfg *models.Slice, charset string, collationID mysql.CollationID, masterDC, forceMaster bool) (*backend.Slice, error) {
	var err error
	s := new(backend.Slice)
	s.Cfg = *cfg
	...
	// parse slaves
	var slaveConfigs []string
	if masterDC || forceMaster {
		slaveConfigs = cfg.Slaves
	} else {
		slaveConfigs = cfg.SlaveDCSlaves
	}

	err = s.ParseMasterDCSlave(cfg.Slaves)
	if err != nil {
		return nil, err
	}

	err = s.ParseSlave(slaveConfigs)
	if err != nil {
		return nil, err
	}
...
func parseSlices(cfgSlices []*models.Slice, charset string, collationID mysql.CollationID, masterDC, forceMaster bool) (map[string]*backend.Slice, error) {
	...
	for _, v := range cfgSlices {
		...
		s, err := parseSlice(v, charset, collationID, masterDC, forceMaster)
		if err != nil {
			return nil, err
		}
