...
func (cc *ClientConn) readHandshakeResponse() (HandshakeResponseInfo, error) {
	...
	log.Debug("capability: %v, %v", capability, capability&mysql.ClientInteractive)
	if capability&mysql.ClientInteractive != 0 {
	...
	if capability&mysql.ClientPluginAuth > 0 {
		...
		authPluginName, pos, ok = mysql.ReadNullString(data, pos)
		if !ok {
			return info, fmt.Errorf("readHandshakeResponse: can't read auth plubin name")
		}
		info.AuthPluginName = authPluginName
	}
...
func (cc *ClientConn) writeColumnDefinition(field *mysql.Field) error {
	...
	length := 4 + // lenEncStringSize("def")
		...
		1 + // decimals
		2 + // filler
		mysql.LenEncIntSize(field.DefaultValueLength) + // default value length
		len(field.DefaultValue) // default value
