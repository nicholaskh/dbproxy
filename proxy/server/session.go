...
var DefaultCapability = mysql.ClientLongPassword | mysql.ClientLongFlag |
	...
	mysql.ClientTransactions | mysql.ClientSecureConnection | mysql.ClientPluginAuth
...
func newSession(s *Server, co net.Conn) *Session {
	cc := new(Session)
	cc.sequence = s.sessionSequencePool.Get()
...
func (cc *Session) Handshake() error {
	...
	if err != nil {
		...
		// FIXME Warn
		log.Notice("[server] Session readHandshakeResponse error, connId: %d, ip: %s, msg: %s, error: %s",
...
func (cc *Session) handleHandshakeResponse(info HandshakeResponseInfo) error {
	...
	if info.Interactive {
		cc.executor.interactive = info.Interactive
	}
	return nil
}
...
func (cc *Session) Run() {
	defer func() {
		...
		cc.Close()
		cc.proxy.tw.Remove(cc.sequence)
	...
	for !cc.IsClosed() {
		...
		cc.proxy.tw.Add(cc.proxy.sessionTimeout, cc.sequence, cc.Close)
...
func (cc *Session) writeResponse(r Response) error {
	...
	case RespError:
		...
		if rs == mysql.ErrBadConn { // 鍚庣杩炴帴濡傛灉鏂紑, 搴旇杩斿洖閫氱煡Session鍏抽棴
			return rs
		}
		return nil
	case RespOK:
...
type sessionSequencePool struct {
	current uint32
	pool    sync.Pool
}

func newSessionSequencePool() *sessionSequencePool {
	ss := sessionSequencePool{}
	ss.pool = sync.Pool{
		New: func() interface{} {
			return atomic.AddUint32(&ss.current, 1)
		},
	}
	return &ss
}

func (ss *sessionSequencePool) Get() uint32 {
	return ss.pool.Get().(uint32)
}

func (ss *sessionSequencePool) Put(i uint32) {
	ss.pool.Put(i)
}
