...
type Server struct {
	...
	manager             *Manager
	EncryptKey          string
	sessionSequencePool *sessionSequencePool
}
...
func NewServer(cfg *models.Proxy, manager *Manager) (*Server, error) {
	...
	s.sessionSequencePool = newSessionSequencePool()
...
func (s *Server) onConn(c net.Conn) {
	...
	defer func() {
		...
		cc.Close()
		s.sessionSequencePool.Put(cc.sequence)
	}()

	if err := cc.Handshake(); err != nil {
		//FIXME Warn
		log.Notice("[server] onConn error: %s", err.Error())
	...
	s.tw.Add(s.sessionTimeout, cc.sequence, cc.Close)
