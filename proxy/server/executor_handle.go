...
func (se *SessionExecutor) doQuery(reqCtx *util.RequestContext, sql string) (*mysql.Result, error) {
	...
	if p != nil {
		defer p.RecycleNodes()
	}
	if err != nil {
		return nil, fmt.Errorf("get plan error, db: %s, sql: %s, err: %v", db, sql, err)
	}

	timeMs64 := time.Now().UnixNano() / 1e6

	flag := getExecuteFlagFromSlave(se, sql, p.GetElapsedTables(), timeMs64)
	reqCtx.Set(util.ExecuteFlag, flag)
	/*if canExecuteFromSlave(se, sql, p.GetElapsedTables(), timeMs64) {
		reqCtx.Set(util.FromSlave, 1)
	}*/

	//如果是写操作 更新下表时间为当前时间
	updateWriteTableCurrentTime(se, sql, p, timeMs64)
...
func (se *SessionExecutor) handleQueryWithoutPlan(reqCtx *util.RequestContext, sql string) (*mysql.Result, error) {
	...
	case *ast.RollbackToStmt:
		return nil, se.handleRollbackTo(stmt.Savepoint)
	case *ast.SavepointStmt:
		return nil, se.handleSavepoint(stmt.Savepoint)
	case *ast.UseStmt:
...
func (se *SessionExecutor) handleFieldList(data []byte) ([]*mysql.Field, error) {
	...
	pc, err := se.getBackendConn(sliceName, se.GetNamespace().GetRWSplit(se.user))
	if err != nil {
		return nil, err
	}
