...
func handleInsertValues(p *InsertPlan) error {
	...
	for _, valueList := range p.stmt.Lists {
		...
		case *driver.ValueExpr:
			...
			if _, ok := valueMap[routeIdx]; !ok {
				indexes = append(indexes, routeIdx)
			}
			valueMap[routeIdx] = append(valueMap[routeIdx], valueList)
		}

	}
	sort.Ints(indexes)
	p.result.Inter(indexes)
	decorator, err := CreateInsertValuesExprDecorator(p.result, valueMap)
	if err != nil {
		return fmt.Errorf("handleInsertValuesExpr error: %v", err)
	}
	p.stmt.ListsAssist = decorator
