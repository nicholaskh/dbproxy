...
type Plan interface {
	...
	//tables
	SetElapsedTables([]string) int

	//tables
	GetElapsedTables() []string

	SetNodesToBeRecycled([]ast.Node)
...
type Checker struct {
	...
	writeElapsedTables []string
	nodesToBeRecycled  []ast.Node
}
...
func (s *Checker) Enter(n ast.Node) (node ast.Node, skipChildren bool) {
	...
	case ast.ValueExpr:
		s.nodesToBeRecycled = append(s.nodesToBeRecycled, nn)
	}
	return n, false
}
...
func (s *Checker) appendElapsedTables(n *ast.TableName) {
	db := n.Schema.L
	if db == "" {
		db = s.db
	}
	table := n.Name.L
	s.writeElapsedTables = append(s.writeElapsedTables, db+":"+table)
}
...
func (bP *basePlan) SetElapsedTables(tables []string) int {
	bP.dbTables = tables
	return 1
}

func (bP *basePlan) GetElapsedTables() []string {
	return bP.dbTables
}
...
func (bP *basePlan) SetNodesToBeRecycled(nodes []ast.Node) {
	bP.nodesToBeRecycled = nodes
}

func (bP *basePlan) RecycleNodes() {
	for _, n := range bP.nodesToBeRecycled {
		driver.RecycleValueExpr(n.(*driver.ValueExpr))
	}
}
...
func BuildPlan(stmt ast.StmtNode, phyDBs map[string]string, db, sql string, router *router.Router, seq *sequence.SequenceManager) (Plan, error) {
	...
	var err error
	if checker.IsShard() {
		plan, err = buildShardPlan(stmt, db, sql, router, seq)
	} else {
		plan, err = CreateUnshardPlan(stmt, phyDBs, db, checker.GetUnshardTableNames())
	}

	if plan != nil {
		plan.SetElapsedTables(checker.writeElapsedTables)
		plan.SetNodesToBeRecycled(checker.nodesToBeRecycled)
	}
...
// Enter for node visit
func (s *Checker) ParseTable(n ast.Node) string {
	switch nn := n.(type) {
	case *ast.TableName:
		if s.isTableNameDatabaseInvalid(nn) {
			s.dbInvalid = true
